...
- 제목·헤드라인·지표 숫자는 명조체(나눔명조), 본문은 고딕.
  ...
  실행하는 컴퓨터에 폰트가 깔려 있지 않아도 결과가 같다.
- 색은 무채색 + 포인트 컬러 한 가지(`--accent`, 진한 적색)만 쓴다.
  지표 등락의 빨강/파랑은 장식이 아니라 정보 전달용이라 예외로 둔다.
- 이모지 아이콘은 쓰지 않는다. 목록·단계는 `01`, `02` … 일련번호로 표시한다.
- `한 줄 요약`은 왼쪽 단 아래에 큰 명조 풀쿼트로 들어가서, 오른쪽
  보조기사 단과 높이를 맞춘다.
- `core_news.image` 는 왼쪽 단 전체 폭으로, `headline.image` 는 기사 왼쪽
  44% 폭으로 들어간다.
...
- `assets/fonts/` — 제목용 명조 웹폰트 (나눔명조, OFL)
- `prompts/` — 그날 삽화를 뽑을 때 쓴 이미지 생성 프롬프트 기록
