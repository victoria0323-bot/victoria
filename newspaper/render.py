...
THEMES = ("magazine", "newspaper", "dark")
SERIF_FONTS = {
...
def render_html(data: dict, theme: str | None = None) -> str:
    data = dict(data)
    data["theme"] = theme or data.get("theme") or "magazine"
...
def main():
    ...
    parser.add_argument("--theme", default=None, choices=THEMES,
                        help="지면 테마 (기본: magazine). newspaper=신문 1면형, dark=다크 모드")
    ...
    if args.out:
        ...
        theme = args.theme or data.get("theme") or "magazine"
        suffix = "" if theme == "magazine" else f"-{theme}"
